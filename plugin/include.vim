...
function! s:AddInclude()
    ...
    if(len(wordParts) > 0)
        ...
        if(wordParts2[0][0] == '&')
            let wordParts2[0] = strpart(wordParts2[0], 1)
        endif
        let tags_tmp = filter(copy(tags), '!has_key(v:val, "namespace") || v:val["namespace"] =~ "'.wordParts2[0].'"')
        let tags_class_exact_match = filter( copy(tags_tmp), 'v:val["kind"] == "c" && has_key(v:val, "namespace") && v:val["namespace"] == "'.wordParts2[0].'"')
